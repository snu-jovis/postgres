	...
	if (join)
	{
		...
		if (IsA(path, MergePath))
		{
			...
			for (i = 0; i < indent; i++)
				printf("\t");
			printf("  details: ");
			printf("sortouter=%d sortinner=%d materializeinner=%d initial_run_cost=%lf initial_startup_cost=%lf inner_scan_cost=%lf inner_startup_cost=%lf outer_run_cost=%lf outer_scan_cost=%lf outer_startup_cost=%lf mergejointuples=%lf bare_inner_cost=%lf mat_inner_cost=%lf merge_eval_cost=%lf merge_init_eval_cost=%lf\n",
				   ((mp->outersortkeys) ? 1 : 0),
				   ((mp->innersortkeys) ? 1 : 0),
				   ((mp->materialize_inner) ? 1 : 0), mp->initial_run_cost, mp->initial_startup_cost, mp->inner_scan_cost, mp->inner_startup_cost, mp->outer_run_cost, mp->outer_scan_cost, mp->outer_startup_cost, mp->mergejointuples, mp->bare_inner_cost, mp->mat_inner_cost, mp->merge_eval_cost, mp->merge_init_eval_cost);
		} 
		/* NestLoop */
